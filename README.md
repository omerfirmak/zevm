...
- **`committed_state_impl` is a compile-time switch.** It's baked into zevm at module-creation time. Passing `.external` *without* doing the `addImport("committed_state", ...)` wiring above will produce an unresolved-import error at compile time. Passing `.empty` with extraneous `committed_state` wiring is harmless — the import is never referenced.
- **Only the module is exported, not a static library.** `b.dependency("zevm", ...).module("zevm")` is the entire public surface. There is no C ABI / `.a` artifact to link.
