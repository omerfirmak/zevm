...
pub fn init(gpa: std.mem.Allocator, bytecode: []const u8, jump_table: [256]ops.Fn) !Bytecode {
    var threaded_code = try gpa.alloc(?ops.Fn, bytecode.len + 1);
    ...
    while (pc < bytecode.len) : (pc += 1) {
        ...
        if (opcode >= @intFromEnum(Opcode.PUSH1) and opcode <= @intFromEnum(Opcode.PUSH32)) {
            ...
            const data_end = @min(bytecode.len, pc + data_len + 1);
...
pub fn isValidJumpDest(self: *Bytecode, pc: u256) ?InstructionPointer {
    ...
    if (self.threaded_code[truncated_pc] == null or
        self.bytecode[truncated_pc] != @intFromEnum(Opcode.JUMPDEST))
    {
        @branchHint(.unlikely);
        return null;
    }
...
pub fn safeSlice(self: *Bytecode, index: u256, size: u64) []const u8 {
    ...
    const read_size = @min(self.bytecode.len - index, size);
    return self.bytecode[@intCast(index)..@intCast(index + read_size)];
}
