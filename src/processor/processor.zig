...
const BUILDER_DEPOSIT_CONTRACT_ADDRESS: u160 = 0x0000BFF46984E3725691FA540A8C7589300D8282;
const BUILDER_EXIT_CONTRACT_ADDRESS: u160 = 0x000064D678505AD48F8CCB093BC65613800E8282;
...
pub fn computeRoot(comptime T: type, gpa: std.mem.Allocator, items: []const T) ![32]u8 {
    ...
    for (ranges) |range| {
        var i = range[0];
        while (i < range[1]) : (i += 1) {
            var key_list = std.array_list.Managed(u8).init(allocator);
            try rlp.serialize(usize, allocator, i, &key_list);
            var val_list = std.array_list.Managed(u8).init(allocator);
            try rlp.serialize(T, allocator, items[i], &val_list);
